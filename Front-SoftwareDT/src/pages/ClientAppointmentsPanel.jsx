...
import { 
  ...
  Activity, ShieldCheck, Database, User, Phone
} from "lucide-react";
...
const ClientAppointmentsPanel = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [apptRes, msgRes] = await Promise.all([
        ...
        axiosPrivate.get(`/messages/user/${userId}`).catch(() => ({ data: { success: true, messages: [] } }))
      ]);
  ...
  // 🛰️ WEBSOCKET-LIKE SYSTEM: Auto-completado autónomo en el cliente sin recargar
  useEffect(() => {
    const checkExpiry = () => {
      const now = new Date();
      ...
      setAppointments((prevAppts) => 
        prevAppts.map((appt) => {
          const isPending = ["pending", "scheduled", "active"].includes(appt.status?.toLowerCase());
          
          if (isPending && appt.appointmentDate) {
            const isPastDate = appt.appointmentDate < todayStr;
            const isTodayButPastTime = appt.appointmentDate === todayStr && appt.appointmentTime < timeStr;

            if (isPastDate || isTodayButPastTime) {
              return { ...appt, status: "completed" }; // Se traslada de pestaña autónomamente
            }
          }
          return appt;
        })
    ...
    const checkerTimer = setInterval(checkExpiry, 60000); // Chequea cada minuto
    checkExpiry(); // Ejecución inicial
  ...
  const handleExternalCommunication = async (type) => {
    ...
    try {
      ...
      if (type === 'WhatsApp') {
        ...
        window.location.href = `mailto:admin@softwaredt.com?subject=Soporte Software DT&body=Hola, requiero asistencia técnica con mi proyecto.`;
      }
  ...
  return (
    <div className="min-h-screen bg-main pb-20 font-sans text-black antialiased overflow-x-hidden">
      ...
      {/* HEADER SDT */}
      <div className="bg-white border-b-2 border-black/5 pt-4 sm:pt-6 pb-6 sm:pb-8 px-4 sm:px-6 md:px-12 shadow-[0_4px_30px_rgba(254,182,13,0.1)]">
        <div className="max-w-[1800px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
            {/* Reloj Responsivo Ampliado */}
            <div className="bg-black text-yellowColor p-5 rounded-xl border-2 border-black flex items-center justify-between sm:justify-center gap-6 shadow-xl w-full sm:w-auto min-w-[280px] overflow-hidden">
               <div className="flex flex-col items-end font-mono flex-1">
                  <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest text-gray-500">System Time</span>
                  <span className="text-xl sm:text-3xl font-black tracking-tight">{currentTime.toLocaleTimeString('en-GB')}</span>
               </div>
               ...
               <div className="flex flex-col flex-1">
                  ...
                  <span className="text-xs sm:text-base font-black uppercase whitespace-nowrap">{currentTime.toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
               </div>
               <Clock size={24} className="animate-pulse text-yellowColor flex-shrink-0 hidden xs:block" />
            </div>

            <Link to="/booking" className="w-full md:w-auto group flex items-center justify-center gap-3 bg-yellowColor text-black border-2 border-black px-6 py-5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all duration-300 hover:-translate-y-1 hover:bg-gold hover:shadow-[0_10px_20px_rgba(255,215,0,0.4)] active:scale-95">
      ...
      <div className="max-w-[1800px] mx-auto px-4 sm:px-6 md:px-12 mt-6 sm:mt-8">
            ...
            <div className="grid gap-5">
              {isLoading ? (
                ...
                filteredAppointments.map((appt) => (
                  <div key={appt._id} className="bg-white border-[3px] border-black/5 rounded-[1.5rem] p-5 sm:p-8 hover:border-gold hover:shadow-[0_15px_30px_rgba(255,215,0,0.15)] transition-all duration-400 group relative overflow-hidden hover:-translate-y-1">
                    <div className={`absolute left-0 top-0 bottom-0 w-2 transition-all ${appt.status === 'completed' ? 'bg-green-500' : 'bg-gold'}`}></div>
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                      <div className="flex-1 space-y-3 w-full">
                        <div className="flex items-center gap-2">
                          ...
                          {appt.status === 'completed' && <span className="text-[8px] font-bold bg-green-500 text-white px-2 py-0.5 rounded uppercase">CUMPLIDA AUTO</span>}
                        </div>
                        <h3 className="text-lg sm:text-3xl font-black uppercase tracking-tighter text-headingColor">{appt.solutionName || appt.serviceName || "Consultoría"}</h3>
                        
                        {/* 🛰️ NUEVA INFORMACIÓN ADICIONAL DE LA CITA */}
                        <div className="flex flex-wrap items-center gap-x-6 gap-y-3 pt-2">
                          <div className="flex items-center gap-2 text-gray-500">
                            <User size={14} className="text-gold" />
                            <span className="text-[11px] font-bold uppercase truncate max-w-[150px]">{appt.clientName || user?.name || "TITULAR"}</span>
                          </div>
                          {appt.clientPhone && (
                            <div className="flex items-center gap-2 text-gray-500">
                              <Phone size={14} className="text-gold" />
                              <span className="text-[11px] font-bold uppercase">{appt.clientPhone}</span>
                            </div>
                          )}
                          <div className="bg-main px-3 py-1.5 rounded-lg font-black text-[10px] sm:text-[12px] flex items-center gap-2">
                            <Clock size={14} className="text-gold" /> {formatDate(appt.appointmentDate)}
                          </div>
                      ...
                      <button onClick={() => navigate("/appointment-confirmation", { state: { appointment: appt } })} className="w-full md:w-auto px-8 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest bg-black text-white hover:bg-gold hover:text-black hover:shadow-lg hover:-translate-y-0.5 transition-all">
                ...
                <div className="bg-white/50 border-2 border-dashed border-black/10 rounded-3xl py-16 text-center">
        ...
        <div className="mt-12 space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
               ...
               <div className="bg-white border-2 border-black/5 rounded-[2.5rem] p-6 sm:p-12 shadow-sm flex flex-col items-center text-center relative overflow-hidden group hover:border-gold/50 transition-all hover:-translate-y-1">
                  <div className="w-20 h-20 sm:w-24 sm:h-24 bg-main rounded-full flex items-center justify-center mb-6 border-4 border-white shadow-xl ring-4 ring-gold/20 overflow-hidden">
                    <div className="w-full h-full overflow-hidden rounded-2xl border-2 border-black/10">
                      <img 
                        ...
                        onError={(e) => { e.target.src = "https://github.com/NietoDeveloper.png"; }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="w-full bg-main/30 border-2 border-black/5 p-4 sm:p-6 rounded-3xl mb-8 flex-grow flex items-center justify-center">
                     <p className="text-[10px] sm:text-[13px] font-bold text-gray-600 leading-relaxed italic">
                       "Protocolo de comunicación activo. Selecciona un canal para comunicación directa"
                     </p>
                  </div>

                  <div className="w-full flex flex-col sm:flex-row gap-4">
                     <button onClick={() => handleExternalCommunication('WhatsApp')}
                        className="flex-1 flex items-center justify-center gap-3 bg-[#25D366] text-white py-4 sm:py-5 rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest hover:bg-gold hover:text-black hover:shadow-lg hover:-translate-y-0.5 transition-all">
                        <MessageCircle size={18} /> WhatsApp
                     </button>
                     <button onClick={() => handleExternalCommunication('Email')}
                        className="flex-1 flex items-center justify-center gap-3 bg-black text-white py-4 sm:py-5 rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest hover:bg-gold hover:text-black hover:shadow-lg hover:-translate-y-0.5 transition-all">
                        <Mail size={18} /> Email
                     </button>
                  </div>
               </div>

               <div className="bg-white border-2 border-black/5 rounded-[2.5rem] flex flex-col shadow-sm hover:border-gold/50 transition-all overflow-hidden h-full hover:-translate-y-1">
                  ...
                  <div className="flex-grow overflow-y-auto max-h-[350px]">
                     <table className="w-full text-left h-full">
                        <tbody className="divide-y divide-gray-50 font-bold text-[9px] sm:text-[10px] uppercase h-full">
                           {messages.length > 0 ? messages.map((msg) => (
                              <tr key={msg._id} className="hover:bg-main/20 transition-colors">
                                 <td className="px-4 sm:px-8 py-5">
                                    <div className="flex items-center gap-2">
                                       <span className={`w-2 h-2 rounded-full ${msg.channel === 'WhatsApp' ? 'bg-[#25D366]' : 'bg-blue-500'}`}></span>
                              ...
                              <tr className="h-full">
                                 <td className="px-8 py-20 text-center text-gray-300 italic align-middle">No hay logs de actividad...</td>
                              </tr>
                           )}
                        </tbody>
                     </table>
