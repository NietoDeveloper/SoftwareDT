    ...
    useEffect(() => {
        ...
        if (bookingData) {
            ...
            setTimeout(() => {
                ...
                console.log("-----------------------------------------");
                console.log(`[Email Mock] 📨 Correo de confirmación enviado a: ${bookingData.doctorName}`);
                console.log(`[Email Mock] 📧 Correo de confirmación enviado al paciente: ${bookingData.patientId}`);
                console.log(`[Email Mock] Cita confirmada: ${bookingData.date} a las ${bookingData.time}`);
                console.log("-----------------------------------------");
    ...
    if (!bookingData) {
        ...
        return (
            <div className="max-w-xl mx-auto p-10 mt-20 bg-red-50 rounded-xl shadow-lg border border-red-300">
                <h1 className="text-3xl font-bold text-red-700">⚠️ Error: Datos de Cita Perdidos</h1>
                <p className="mt-4 text-red-600">
                    No se encontraron los detalles de la cita. Esto puede pasar si se refresca la página. 
                    Por favor, verifica el estado en la sección de "Mis Citas".
                </p>
                <button
            ...
            <div className="max-w-3xl mx-auto bg-white shadow-2xl rounded-3xl p-8 sm:p-12 transition-all duration-300">
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid sm:grid-cols-3 gap-4">
                        ...
                        <DetailCard title="Especialidad" value={specialization || "General"} />
                        <DetailCard title="Fecha" value={new Date(date).toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} />
                        <DetailCard title="Hora" value={time} />
                        <DetailCard title="ID Paciente" value={bookingData.patientId} />
                        <DetailCard title="Estado" value={bookingData.status} className="text-green-600" />
                    </div>
                </div>

                {/* Estado de las Notificaciones (Mails) */}
                <div className="mt-10 p-5 bg-gray-50 rounded-xl border border-gray-200">
                    <h3 className="text-xl font-bold text-gray-800 mb-4">Estado de Notificaciones</h3>
                    <div className="space-y-3">
                        <EmailStatusItem recipient="Paciente" status={emailStatus.user} />
                        <EmailStatusItem recipient="Doctor/Programador" status={emailStatus.doctor} />
                    </div>
                </div>
...
const DetailCard = ({ title, value, className = '' }) => (
    <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100">
        ...
        <p className={`text-lg font-semibold text-gray-900 truncate ${className}`}>{value}</p>
    </div>
);

// Componente auxiliar para el estado del correo
const EmailStatusItem = ({ recipient, status }) => {
    const color = status === 'Enviado con éxito' ? 'text-green-600' : 'text-yellow-600';
    const text = status === 'Enviado con éxito' ? 'Enviado con éxito' : 'Enviando...';
    
    const icon = status === 'Enviado con éxito' ? (
    ...
    return (
        <div className="flex justify-between items-center py-2 border-b last:border-b-0">
            <span className="text-gray-700 font-medium flex items-center">{icon} Notificación a {recipient}</span>
