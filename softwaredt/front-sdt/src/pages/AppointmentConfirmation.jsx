...
const AppointmentConfirmation = () => {
    ...
    useEffect(() => {
        if (bookingData) {
            // Simula guardado en DB
            const timerSave = setTimeout(() => {
                try {
                    // Aquí simula éxito o error (para testing, siempre éxito)
                    console.log('Cita simulada guardada:', bookingData);
                    setSaveStatus("Guardado con éxito");

                    // Simula envío emails después
                    setTimeout(() => {
                        setEmailStatus({
                    ...
                    setSaveStatus("Error");
                    setErrorMessage("Error simulado al guardar la cita.");
                }
            }, 1000); // Simula delay de API

            return () => clearTimeout(timerSave);
        }
    }, [bookingData]);
    ...
    if (saveStatus === "Error") {
        return (
            <div className="max-w-xl mx-auto p-10 mt-20 bg-red-50 rounded-xl shadow-lg border border-red-300">
                ...
                <p className="mt-4 text-red-600">
                    {errorMessage || "Ocurrió un error al guardar la cita en la base de datos. Intenta nuevamente."}
                </p>
                <button
                    onClick={() => navigate("/services")}
                    className="mt-6 py-2 px-4 bg-red-500 text-white rounded-lg hover:bg-red-600 transition shadow-md hover:shadow-lg"
