...
// 1. Añadimos useLocation para obtener los datos pasados desde DoctorList
import { useParams, useNavigate, useLocation } from 'react-router-dom'; 
import { AppContext } from '../context/UserContext';
// import { axiosAuth } from '../API/api'; // Si estuvieras haciendo llamadas reales

// --- Componentes SVG para Iconografía (Sin cambios) ---
...
// Mock de disponibilidad de franjas horarias (Sin cambios)
const MOCK_SLOTS = [
...
    const location = useLocation(); // 👈 Hook para acceder a los datos de navegación
    const { user } = useContext(AppContext);

    // 2. Extraer los datos del doctor del objeto 'location.state'
    // Si no hay datos (ej: refrescar la página), usamos un objeto vacío para evitar errores.
...
    const [availableSlots, setAvailableSlots] = useState([]);
    // 3. Inicializar el nombre del doctor con el dato real o un fallback
    const [doctorName, setDoctorName] = useState(doctor.name || "Dr. Desconocido (ID: " + doctorId + ")");
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const [message, setMessage] = useState(null);

    // 4. ELIMINAMOS el antiguo useEffect con la lógica de Mock de doctorId, 
    //    ya que estamos usando los datos pasados por 'state'.
    /* useEffect(() => {
        setTimeout(() => {
...
    // Función Mock: Carga de franjas horarias disponibles (Sin cambios)
...
    // Efecto para cargar slots cuando cambia la fecha (Sin cambios)
    useEffect(() => {
        if (selectedDate) {
...
                status: 'Confirmada', // 5. Cambiar a 'Confirmada' ya que estamos saltando el pago
                // Añadimos el nombre del doctor para mostrarlo en la confirmación
...
            
            // 6. Redirigir a una página de confirmación, saltando el pago.
...
    };
    
    // Obtener la fecha mínima (hoy) para el input date (Sin cambios)
...
                </p>
                {/* ... (Resto del JSX sin cambios, ya que ahora doctorName se carga correctamente) ... */}
                
