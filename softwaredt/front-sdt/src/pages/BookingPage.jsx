...
  const { doctorId } = useParams();
  const location = useLocation(); // Hook para acceder al state de navegación
  
  // MODIFICADO: Intenta obtener los datos del doctor desde el estado de navegación
...
    let isMounted = true;
    ...
    // Si ya tenemos el objeto del doctor (porque fue pasado por navegación), no hacemos el fetch inicial
...
      isMounted = false;
    };
  }, [doctorId, token, doctor]); // Añadido 'doctor' para el control de la carga inicial
...
                <button className="btn" type="submit">Agenda Cita</button>
              </form>
            </div>
          </div>
        </div>
...
        </div>
       ) : (
        <p className="text-center text-red-500">Cargando Perfil del Programador...</p>
       )}
      </div>
     
    </div>
