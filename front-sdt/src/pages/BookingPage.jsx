...
const BookingPage = () => {
  ...
  if (isLoading) return <div className="flex items-center justify-center min-h-screen font-bold text-black">Cargando detalles...</div>;

  if (error || !doctor) return (
    <div className="flex flex-col items-center justify-center min-h-screen">
      <h1 className="text-red-500 font-bold text-xl">Error al cargar el servicio.</h1>
      <button onClick={() => navigate("/doctors")} className="mt-4 border-2 border-black px-6 py-2 rounded-lg hover:bg-black hover:text-white transition-all font-bold uppercase text-xs">Volver</button>
    </div>
  );
            ...
            <div className="bg-white p-8 rounded-lg shadow-lg border border-gray-100 w-full lg:max-w-[600px]">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <textarea name="reason" value={formData.reason} onChange={handleInputChange} required minLength={10} className="w-full border border-black p-4 rounded-lg h-32 focus:ring-2 focus:ring-amber-500 outline-none resize-none bg-white text-black" placeholder="Indica brevemente el motivo de tu cita..." />
                </div>
