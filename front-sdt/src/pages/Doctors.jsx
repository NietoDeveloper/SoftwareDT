...
const DoctorList = () => {
  ...
  const getDoctors = async () => {
    try {
      ...
      const res = await axios.get(`${apiUrl}/doctors?_cache=${Date.now()}&random=${Math.random()}`, {  // Doble buster para forzar
        headers: {
          'Cache-Control': 'no-cache, no-store, must-revalidate, max-age=0',  // Fix cliente para 304
          'Pragma': 'no-cache',
          'Expires': '0',
          'If-None-Match': '',  // Evita ETag check
          'If-Modified-Since': ''  // Evita modified check
        }
      });
      console.log('API response (debe ser 200 con datos):', res.status, res.data);  // Debug frontend
      return res.data.doctors || res.data || [];
    } catch (error) {
      if (error.response) {
        toast.error(`Error ${error.response.status}: ${error.response.data.message || 'Fallo desconocido.'}`);
      } else {
  ...
  const { data: doctors = [], error, isLoading } = useQuery({
    ...
    queryFn: getDoctors,
    initialData: [],
    staleTime: 0,  // Reduce a 0 para forzar refetch siempre en dev
  ...
  if (doctors.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-black mt-2">Por favor, inténtalo de nuevo más tarde o verifica la DB.</p>
      </div>
    );
  }
