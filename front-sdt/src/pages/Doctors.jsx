...
const DoctorList = () => {
  ...
  const getDoctors = async () => {
    try {
      ...
      const res = await axios.get(`${apiUrl}/doctors?_cache=${Date.now()}`, {
        headers: {
          'Cache-Control': 'no-cache, no-store, must-revalidate',  // Agregado: fuerza no-cache en cliente
          'Pragma': 'no-cache',
          'Expires': '0'
        }
      });
      console.log('Respuesta API (debería ser 200):', res.status, res.data);  // Debug: ve en consola frontend
