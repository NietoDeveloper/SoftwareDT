...
const Header = () => {
  ...
  return (
    <header className="bg-white/10 backdrop-blur-2xl shadow-sm sticky top-0 z-[100] border-b border-black/5">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex-shrink-0 flex items-center cursor-pointer group" onClick={handleLogoClick}>
            {/* H2 ajustado: más grande en móvil y con brillo gold (drop-shadow) */}
            <h2 className="text-black text-2xl sm:text-2xl font-black uppercase tracking-tighter transition-all duration-300 group-hover:text-[#FEB60D] drop-shadow-[0_0_8px_rgba(255,215,0,0.6)]">
              Software D T
            </h2>
          </div>
      ...
      {/* Mobile Menu ajustado con Hovers Gold */}
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
          ...
          >
            <div className="px-6 py-8 space-y-4 flex flex-col items-center">
              ...
              <Link to="/services" onClick={() => setIsMenuOpen(false)} className="text-lg font-black transition-colors duration-300 hover:text-[#FFD700]">Información Servicios</Link>
              ...
              <Link to="/clients" onClick={() => setIsMenuOpen(false)} className="text-lg font-black transition-colors duration-300 hover:text-[#FFD700]">Nuestros Clientes</Link>
              ...
              {isLoggedIn ? (
                <button 
                  ...
                  className="text-lg font-black text-red-600 transition-colors duration-300 hover:text-[#FFD700]"
                >
                  Cerrar Sesión
                </button>
              ) : (
                <>
                  ...
                  <Link to="/signup" onClick={() => setIsMenuOpen(false)} className="text-lg font-black text-[#FEB60D] transition-colors duration-300 hover:text-[#FFD700]">Registro</Link>
                </>
              )}
            </div>
          </motion.div>
